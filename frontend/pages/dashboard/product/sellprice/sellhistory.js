...
const Viewselltracking = () => {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/product/getselltracking', {
        headers: {
      ...
      console.error('Error fetching product sell tracking list:', error);
      setError('Error fetching product sell tracking list. Please try again.');
      setLoading(false);
    }
  };
  ...
  const handleDeleteAllData = async () => {
    try {
      ...
      await axios.delete('http://localhost:3000/product/deleteselltracking', {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
  ...
  if (product.length === 0) {
    return <div className="text-center text-red-700">Zero Product Buy history has found.</div>;
  }
...
export default Viewselltracking;
